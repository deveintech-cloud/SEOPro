    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-slate-800">Readability Analyzer</h2>
      ...
      {result && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center justify-center">
            <h3 className="font-semibold text-slate-800 mb-2">Reading Ease Score</h3>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                  <div className="text-xs text-slate-500 uppercase font-semibold">Grade Level</div>
                  <div className="text-2xl font-bold text-indigo-600">{result.gradeLevel}</div>
               </div>
               <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                  <div className="text-xs text-slate-500 uppercase font-semibold">Words</div>
                  <div className="text-2xl font-bold text-slate-700">{result.wordCount}</div>
               </div>
               <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                  <div className="text-xs text-slate-500 uppercase font-semibold">Avg. Sent. Len</div>
                  <div className={`text-2xl font-bold ${result.avgSentenceLength > 20 ? 'text-amber-500' : 'text-emerald-500'}`}>
                    {result.avgSentenceLength}
                  </div>
               </div>
               <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                  <div className="text-xs text-slate-500 uppercase font-semibold">Complex Words</div>
                  <div className="text-2xl font-bold text-slate-700">{result.complexWords}</div>
               </div>
            </div>
