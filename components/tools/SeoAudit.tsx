    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-slate-800">On-Page SEO Audit</h2>
          
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button
              onClick={() => { setMode('url'); setInput(''); setResult(null); }}
              className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'url' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              URL Audit
            </button>
            <button
              onClick={() => { setMode('source'); setInput(''); setResult(null); }}
              className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'source' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
            >
              Paste HTML
            </button>
          </div>
        ...
        <form onSubmit={handleAudit} className="space-y-4">
          {mode === 'url' ? (
            ...
            <textarea
                ...
                className="w-full h-48 px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none font-mono text-xs"
                required
            />
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 text-center">
              <h3 className="text-slate-500 font-medium mb-4">Overall Health</h3>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <h3 className="font-semibold text-slate-800 mb-4">Core Vitals (Est.)</h3>
                <div className="space-y-4">
                    <div>
                        <div className="flex justify-between text-sm mb-1">
                            <span>Load Time</span>
                            <span className={result.loadTime < 1.5 ? 'text-green-600' : 'text-red-500'}>{result.loadTime}s</span>
                        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 border-b border-slate-100">
                <h3 className="font-bold text-lg text-slate-800">Audit Report</h3>
            </div>
            <div className="divide-y divide-slate-100">
                        ...
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1">
                                <h4 className={`font-semibold text-sm ${
                                    ...
                                    issue.type === 'warning' ? 'text-amber-700' : 'text-red-700'
                                }`}>
                                    {issue.label}
                                </h4>
                                ...
                                {issue.recommendation && (
                                    <div className="mt-3 flex items-start gap-2 bg-blue-50 p-3 rounded-md border border-blue-100">
                                        <Lightbulb className="text-blue-500 shrink-0 mt-0.5" size={16} />
                                        <div>
                                            <span className="text-xs font-bold text-blue-700 uppercase tracking-wide">Solution: </span>
                                            <span className="text-sm text-blue-800">{issue.recommendation}</span>
                                        </div>
                                    </div>
                                )}
                            </div>
             ...
             <div className="p-4 bg-slate-50 text-center">
                <button className="text-brand-600 text-sm font-medium hover:underline flex items-center justify-center gap-1 mx-auto">
